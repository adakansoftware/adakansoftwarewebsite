...
export function AdminContentManager({ email }: { email: string }) {
  ...
  return <div className="mx-auto min-h-screen max-w-6xl px-5 py-10 sm:px-8">
    ...
    <div className="mb-5 flex flex-col gap-3 sm:flex-row"><input className="h-10 flex-1 rounded-md border border-border/60 bg-background/50 px-3 text-sm" placeholder="İçerik ara…" value={query} onChange={(event) => setQuery(event.target.value)} /><select className="h-10 rounded-md border border-border/60 bg-background/50 px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">Tümü</option><option value="published">Yayında</option><option value="draft">Taslak</option><option value="archived">Arşiv</option></select></div>
