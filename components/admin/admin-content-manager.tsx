...
export function AdminContentManager({ email }: { email: string }) {
  ...
  const setArchived = async (row: ContentRow, archived: boolean) => {
    const { error } = kind === "projects"
      ? await getSupabaseBrowserClient().from("projects").update({ archived } as never).eq("id", row.id)
      : await getSupabaseBrowserClient().from("logo_works").update({ archived } as never).eq("id", row.id)
    setMessage(error ? error.message : archived ? "Arşive taşındı." : "Arşivden çıkarıldı.")
    await load()
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-6xl px-5 py-10 sm:px-8">
      ...
      <div className="mb-8 grid gap-3 sm:grid-cols-3">
        {[{ label: "Yayında", value: stats.published }, { label: "Taslak", value: stats.draft }, { label: "Arşiv", value: stats.archived }].map((item) => <div key={item.label} className="rounded-xl border border-border/50 bg-card/25 p-4"><p className="text-xs text-muted-foreground">{item.label}</p><p className="mt-1 text-2xl font-bold">{item.value}</p></div>)}
      </div>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row"><input className="h-10 flex-1 rounded-md border border-border/60 bg-background/50 px-3 text-sm outline-none focus:border-primary" placeholder="İçerik ara…" value={query} onChange={(event) => setQuery(event.target.value)} /><select className="h-10 rounded-md border border-border/60 bg-background/50 px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">Tümü</option><option value="published">Yayında</option><option value="draft">Taslak</option><option value="archived">Arşiv</option></select></div>

      <div className="grid gap-8 lg:grid-cols-[1fr_1.15fr]">
        <div className="space-y-3">
          {visibleRows.map((row) => <div key={row.id} className="rounded-xl border border-border/50 bg-card/25 p-4"><div className="flex items-start justify-between gap-3"><button className="text-left" onClick={() => setEditing(row)}><p className="font-semibold">{row.title_tr}</p><p className="mt-1 text-xs text-muted-foreground">{row.archived ? "Arşiv" : row.published ? "Yayında" : "Taslak"} · Sıra {row.sort_order}</p></button><div className="flex gap-1">{row.published && !row.archived && <Button size="icon-sm" variant="ghost" aria-label="Sitede önizle" onClick={() => window.open(kind === "projects" ? "/projects" : "/logo", "_blank")}><Eye /></Button>}<Button size="icon-sm" variant="ghost" aria-label={row.archived ? "Arşivden çıkar" : "Arşivle"} onClick={() => void setArchived(row, !row.archived)}>{row.archived ? <RotateCcw /> : <Archive />}</Button><Button size="icon-sm" variant="ghost" aria-label="Kalıcı sil" onClick={() => void remove(row.id)}><Trash2 /></Button></div></div></div>)}
          {!visibleRows.length && <p className="rounded-xl border border-dashed border-border/60 p-5 text-sm text-muted-foreground">Bu filtrede kayıt yok.</p>}
        </div>
        <form onSubmit={save} className="rounded-2xl border border-border/50 bg-card/25 p-5 sm:p-7">
          ...
          {file && <Image unoptimized className="mt-4 aspect-video w-full rounded-xl border border-border/50 object-contain" src={URL.createObjectURL(file)} alt="Yeni görsel önizlemesi" width={960} height={540} />}
