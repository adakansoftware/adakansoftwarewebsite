...
test("rejects a validly signed session for a different admin email", () => {
  const session = createAdminSession(email, secret, 10_000)

  assert.equal(verifyAdminSession(session, "other-admin@example.com", secret, 5_000), false)
})

test("rejects a signed session with a fractional expiry timestamp", () => {
  const session = createAdminSession(email, secret, 10_000.5)

  assert.equal(verifyAdminSession(session, email, secret, 5_000), false)
})
